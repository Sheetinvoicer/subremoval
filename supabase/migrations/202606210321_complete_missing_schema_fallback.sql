...
alter table public.user_settings add column if not exists default_currency text default 'USD';
...
alter table public.user_settings add column if not exists company_email text;
alter table public.user_settings add column if not exists company_phone text;
alter table public.user_settings add column if not exists company_address text;
...
alter table public.time_entries add column if not exists date date default now()::date;
alter table public.time_entries add column if not exists entry_date date default now()::date;
alter table public.time_entries add column if not exists hours numeric(10,2);
...
begin
  ...
  if not exists (
    select 1 from pg_constraint
    where conname = 'time_entries_duration_minutes_positive'
      and conrelid = 'public.time_entries'::regclass
  ) then
    alter table public.time_entries
      add constraint time_entries_duration_minutes_positive check (duration_minutes is null or duration_minutes > 0);
  end if;
end $$;
...
drop trigger if exists user_settings_sync_before_write on public.user_settings;
...
begin
  ...
  if not exists (
    select 1 from pg_policies where schemaname = 'public' and tablename = 'user_settings' and policyname = 'Users can delete own settings'
  ) then
    create policy "Users can delete own settings" on public.user_settings for delete using (auth.uid() = user_id);
  end if;

  if not exists (
    select 1 from pg_policies where schemaname = 'public' and tablename = 'projects' and policyname = 'Users can select own projects'
  ) then
  ...
  if not exists (
    select 1 from pg_policies where schemaname = 'public' and tablename = 'time_entries' and policyname = 'Users can select own time entries'
  ) then
    create policy "Users can select own time entries" on public.time_entries for select using (auth.uid() = user_id);
  end if;
  ...
  if not exists (
    select 1 from pg_policies where schemaname = 'public' and tablename = 'time_entries' and policyname = 'Users can delete own time entries'
  ) then
