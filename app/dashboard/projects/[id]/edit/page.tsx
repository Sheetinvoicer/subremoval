...
export default function EditProjectPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      if (!user) {
        setError(t('edit.errors.loginRequired'));
        return;
      }
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!project.name.trim()) {
      setError(t('edit.errors.nameRequired'));
      return;
    }
    ...
    if (!supabase) {
      setSaving(false);
      setError(t('edit.errors.supabaseInit'));
      return;
    }
  ...
  return (
    <div className="p-6 md:p-8 max-w-2xl">
      <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-6">{t('edit.title')}</h1>
      ...
      {project && (
        <form onSubmit={onSubmit} className="space-y-4 bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-200 dark:border-gray-700">
          <input value={project.name} onChange={(e) => setProject({ ...project, name: e.target.value })} placeholder={t('edit.projectNamePlaceholder')} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-transparent px-3 py-2" />
          <textarea value={project.description || ''} onChange={(e) => setProject({ ...project, description: e.target.value })} placeholder={t('edit.descriptionPlaceholder')} rows={4} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-transparent px-3 py-2" />
          ...
          <select value={project.client_id || ''} onChange={(e) => setProject({ ...project, client_id: e.target.value || null })} className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-transparent px-3 py-2">
            <option value="">{t('edit.noClient')}</option>
            {clients.map((client) => (
          ...
          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={saving} className="bg-purple-600 hover:bg-purple-700 text-white px-5 py-2 rounded-lg disabled:opacity-60">
              {saving ? t('edit.saving') : t('edit.saveProject')}
            </button>
            <Link href={`/dashboard/projects/${project.id}`} className="px-5 py-2 rounded-lg border border-gray-300 dark:border-gray-600">{t('edit.cancel')}</Link>
          </div>
        </form>
      )}
    </div>
